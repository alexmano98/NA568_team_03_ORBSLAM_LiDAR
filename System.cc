...
Sophus::SE3f readSE3fFromFile(const std::string& filename)
{
    ...
    // Initialize Tcw to identity matrix
    ...
    while (std::getline(file, line)) {
        // Parse the 12 values from the line
        ...
        // Convert the matrix to an SE3 object
        Sophus::SE3f Tcw(matrix.block<3,3>(0,0), matrix.block<3,1>(0,3));

        // Update Tcw by multiplying with T
    }
